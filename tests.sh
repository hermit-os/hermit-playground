...
for f in $FILES; do echo "check $f..."; qemu-system-x86_64 -display none -smp 1 -m 64M -serial stdio -kernel /work/loader/target/x86_64-unknown-hermit-loader/debug/rusty-loader -initrd $f -cpu qemu64,apic,fsgsbase,rdtscp,xsave,fxsr || exit 1; done

for f in $FILES; do echo "check $f..."; qemu-system-x86_64 -display none -smp 2 -m 64M -serial stdio -kernel /work/loader/target/x86_64-unknown-hermit-loader/debug/rusty-loader -initrd $f -cpu qemu64,apic,fsgsbase,rdtscp,xsave,fxsr || exit 1; done
